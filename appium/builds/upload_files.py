...
data = runCommandJson("/Users/$(whoami)/bin/aws devicefarm create-upload --project-arn arn:aws:devicefarm:us-west-2:655603536603:project:fc6f6e31-7c1e-49dd-b212-03739b00dcb1 --name Univision_com.univision.android.apk --type ANDROID_APP")
arnApp = data["upload"]["arn"]
urlApp = data["upload"]["url"]
runCommand('curl -T Univision_com.univision.android.apk '+urlApp)
data = runCommandJson('/Users/$(whoami)/bin/aws devicefarm create-upload --project-arn arn:aws:devicefarm:us-west-2:655603536603:project:fc6f6e31-7c1e-49dd-b212-03739b00dcb1 --name test.zip --type APPIUM_JAVA_TESTNG_TEST_PACKAGE')
...
data = runCommandJson('/Users/$(whoami)/bin/aws devicefarm schedule-run --project-arn arn:aws:devicefarm:us-west-2:655603536603:project:fc6f6e31-7c1e-49dd-b212-03739b00dcb1 --app-arn '+arnApp+' --device-pool-arn '+devicePool+' --execution-configuration '+executionConfiguration+' --test '+testStructure)
print(data)
